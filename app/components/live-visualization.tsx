...
export default function LiveVisualization({ audioData }: LiveVisualizationProps) {
  ...
  const initializeAudioAnalysis = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("AudioContext creation failed, using mock data:", contextError)
        const mockData = generateMockAnalysisData()
        setLiveData(mockData)
        drawMockVisualizations(mockData)
        return
      ...
      try {
        ...
        console.warn("Audio node connection failed, using mock data:", connectionError)
        audioContext.close()
        const mockData = generateMockAnalysisData()
        setLiveData(mockData)
        drawMockVisualizations(mockData)
      ...
      return () => {
        try {
          ...
          if (audioRef.current) {
            delete (audioRef.current as any).audioContext
          }
        } catch (cleanupError) {
  ...
  useEffect(() => {
    return () => {
      ...
      if (audioRef.current && (audioRef.current as any).audioContext) {
        try {
